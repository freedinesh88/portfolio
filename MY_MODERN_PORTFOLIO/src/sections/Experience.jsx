...
function ExperienceItem({exp,idx,start,end,scrollYProgress,layout}){
    ...
    if(layout==="desktop"){
        return(
            <div className="relative flex flex-1 justify-center min-w-o">
                <motion.div className="z-10 w-7 h-7 rounded-full bg-white shadow-[0_0_0_8px_rgba(255,255,255,0.1)]"
                style={{scale,opacity}}>
                
                </motion.div>
                <motion.div className={`absolute ${idx%2 ===0 ? "-top-18" : "-bottom-8"} w-[3px] bg-white/40`}
                style={{height: 40, opacity}}>
                
                </motion.div>
                <motion.article className={`absolute ${idx%2===0 ? "bottom-12" : "top-12"}
                bg-grey-900/80 backdrop-blur border border-grey-700/70 rounded-xl p-7 w-[320px] shadow-lg`}
                style={{opacity, y, maxWidth:"90vw"}}
                transition={{duration: 0.4 , delay: idx*0.15}}>
                <h3 className="text-xl font-semibold">
                {exp.role}
                </h3>
                <p className="text-md text-grey-400 mb-3">
                {exp.company} | {exp.duration}
                </p>
                <p className="text-md text-grey-300 break-words">
                {exp.description}
                </p>
    ...
    return(
        ...
        <motion.article className="bg-grey-900/80 backdrop-blur border border-grey-700/70 rounded-xl p-5 w-[90vw] max-w-sm ml-6 shadow-lg"
        style={{opacity,x}}
        transition={{duration: 0.4 , delay: idx*0.15}}>
        <h3 className="text-lg font-semibold break-words">
                {exp.role}
                </h3>
                <p className="text-sm text-grey-400 mb-2 break-words">
                {exp.company} | {exp.duration}
                </p>
                <p className="text-sm text-grey-300 break-words">
                {exp.description}
                </p>
